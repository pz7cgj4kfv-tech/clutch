...
// ⚠️ V1 = couplage local avec une estimation grossière du trajet (vol d'oiseau × 1.35 ÷ 30 km/h).
...
// Trajet estimé (ms) depuis une distance km : vol d'oiseau × 1.35 ÷ 30 km/h.
...
export const CONE_BUFFER_MIN = 15
...
export const CONE_TENSION_BAND_MIN = 60
...
// Inverse de travelMs : km = (ms / 3.6e6) × 30 / 1.35.
export function credibleRadiusKm(now: number, startAt: number, bufferMin = CONE_BUFFER_MIN): number {
  ...
  return (availMs / 3_600_000) * 30 / 1.35
}
...
// Facteur km→minutes de trajet (inverse de travelMs en minutes) : 1 km = 1.35/30 h = 2.7 min.
const KM_TO_TRAVEL_MIN = (1.35 / 30) * 60
